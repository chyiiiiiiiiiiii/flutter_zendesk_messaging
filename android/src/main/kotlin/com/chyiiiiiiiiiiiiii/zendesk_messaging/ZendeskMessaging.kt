...
class ZendeskMessaging(
    ...
    fun show() {
        try {
            ...
            Log.d(TAG, "Show messaging UI (default behavior)")
        } catch (e: Exception) {
        ...
        try {
            when (event) {
                ...
                is ZendeskEvent.AuthenticationFailed -> {
                    channel.invokeMethod(
                        ON_EVENT,
                        mapOf("type" to "authentication_failed", "error" to event.error.message)
                    )
                }
                ...
                is ZendeskEvent.FieldValidationFailed -> {
                    ...
                    channel.invokeMethod(
                        ON_EVENT,
                        mapOf("type" to "field_validation_failed", "errors" to errors)
                    )
                }
                ...
                is ZendeskEvent.SendMessageFailed -> {
                    channel.invokeMethod(
                        ON_EVENT,
                        mapOf("type" to "send_message_failed", "error" to event.cause.message)
                    )
                }
